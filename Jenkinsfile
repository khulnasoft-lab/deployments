...
pipeline {
    ...
    environment {
        ...
        KHULNASOFTDEV_AZURE_ACR_PASSWORD = credentials('khulnasoftdevAzureACRpassword')
        ...
        DOCKER_HUB_USERNAME = 'khulnasoftautomationci'
        DOCKER_HUB_PASSWORD = credentials('khulnasoftautomationciDockerHubToken')
        DEPLOY_REGISTRY = "khulnasoft.azurecr.io"
    }
    stages {
        stage("Checkout") {
            steps {
            ...
            steps {
                script {
                    ...
                    deploymentImage.inside("-u root") {
                        ...
                        sh """
                        aws codeartifact login --tool pip --repository deployment --domain khulnasoft-deployment --domain-owner ${AWS_ACCOUNT_ID}
                        pip install khulnasoft-deployment
                        """
            ...
            steps{
                script{
                    ...
                    deploymentImage.inside("-u root --network host") {
                        log.info "Pulling manifests with Khulnasoftctl and modifying other manifests"
                        sh """
                        aws codeartifact login --tool pip --repository deployment --domain khulnasoft-deployment --domain-owner ${AWS_ACCOUNT_ID}
                        pip install khulnasoft-deployment
