#!/bin/bash
# Simple shell script to generate required SSL certificates to be used with Khulnasoft KubeEnforcer
_namespace() {
    if [[ -z "$1" ]]
    then
        ns="khulnasoft"
...
_banner() {
    echo
    echo "In this script you will configure and deploy Khulnasoft KubeEnforcer config to your kubernetes cluster"
    echo
    echo "This script will: "
    echo " * Generate SSL certs signed by private root CA bundle"
    echo " * Download and prepare Khulnasoft KubeEnforcer admission controller manifest"
    echo " * Deploy Khulnasoft KubeEnforcer admission controller (if needed)"
    echo
    echo
    printf "PROCEED? [y/N] "
...
_generate_ssl() {
    ...
    if openssl genrsa -out khulnasoft_ke.key 2048; then
        printf "\nInfo: Successfully generated khulnasoft_ke.key\n"
        ...
        [ alt_names ]
        DNS.1 = khulnasoft-kube-enforcer.$ns.svc
        DNS.2 = khulnasoft-kube-enforcer.$ns.svc.cluster.local
        [ v3_req ]
...
EOF
        ...
        if openssl req -new -sha256 -key khulnasoft_ke.key -subj "/CN=khulnasoft-kube-enforcer.$ns.svc" -config server.conf -out khulnasoft_ke.csr; then
            printf "\nInfo: Successfully generated khulnasoft_ke.csr\n"
            printf "\nInfo: Generating kubeEnforcer certificate\n"
            if openssl x509 -req -in khulnasoft_ke.csr -CA rootCA.crt -CAkey rootCA.key -CAcreateserial -out khulnasoft_ke.crt -days 365 -sha256 -extensions v3_req -extfile server.conf; then
                printf "\nInfo: Successfully generated khulnasoft_ke.crt\n"
                _prepare_ke
            else
...
_prepare_ke() {
    ...
    elif curl https://raw.githubusercontent.com/khulnasoft-lab/deployments/$githubBranch/enforcers/kube_enforcer/kubernetes_and_openshift/manifests/kube_enforcer_advanced_starboard/001_kube_enforcer_config.yaml -o "001_kube_enforcer_config.yaml"; then
        _addCABundle=$(sed -i'.original' "s/caBundle.*/caBundle\:\ $_rootCA/g" "$script_dir/001_kube_enforcer_config.yaml")
        if eval "$_addCABundle"; then
            printf "\nInfo: Successfully prepared 001_kube_enforcer_config.yaml manifest file.\n"
            _deploy_ke_admin
